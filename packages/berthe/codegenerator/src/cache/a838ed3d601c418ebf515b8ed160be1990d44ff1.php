    <h1 class="text-danger">List of <?php echo e(ucfirst($config->getPluralForm($table['title']))); ?></h1>
    ...
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title"><?php echo e(ucfirst($config->getPluralForm($table["title"]))); ?></h2>
        </header>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table table-striped mb-none" id="datatable-default">
                    <thead>
                        <tr>
                        <?php $__currentLoopData = $table['attributs']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attrName => $attrType): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?> <?php if($attrType->isDisplayable() && $attrType->isDisplayed()): ?>
                    ...
                    <tbody>
                        S3Bforelse($<?php echo e($table['title'].'s'); ?> as $<?php echo e($table['title']); ?>)
                            ...
                            <?php $__currentLoopData = $table['attributs']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attrName => $attrType): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?> <?php if($attrType->isDisplayable() && $attrType->isDisplayed()): ?>
                                <!--class="{$attrType->formClass("table")}}"-->
                                <td class="text-md">S2BOBRACKET$<?php echo $table['title'].'->'.$attrName; ?>S2BCBRACKET <?php echo e($attrType->getUnit()); ?></td>
                            <?php endif; ?> <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); ?>
                                ...
                                <td class="text-md">
                                    <form action="S2BOBRACKET<?php echo "url(\"/".$table['title']."/related/$".$table['title'].'->'.$table['id']."\")"; ?>S2BCBRACKET" method="get">
                                        ...
                                        <button type="submit" class="btn btn-link" data-toggle="tooltip" data-placement="top" title="<?php echo e($config->getHoverMessage($table['title'].$relation->getOtherTable())); ?>"><?php echo ucfirst($config->getPluralForm($relation->getOtherTable())); ?></button>
                                    </form>
                                </td>
                            <?php endif; ?> <?php endforeach; $__env->popLoop(); $loop = $__env->getFirstLoop(); ?> <?php endif; ?>
