...
<h1 class="text-danger">Edit Actor</h1>
    <form method="post" action="{{url("actor/$actor->actor_id")}}">
        ...
        <select id="film" name="film[]"  multiple="multiple" size="10" >
             @forelse(\App\Film::paginate(5000)->sortBy('title') as  $film)                 <option value="{{$film->film_id}}" @foreach($actor->film as  $filmtmp) @if($filmtmp->film_id == $film->film_id) selected = "selected" @endif @endforeach>
                    {{$film->title}}
                </option>
            @empty
