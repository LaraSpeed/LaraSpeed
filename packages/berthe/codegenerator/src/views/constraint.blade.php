...
@section('constraints')@foreach($tbs as $tabName => $table)@if(array_key_exists('relations', $table))
...
@section('dropTables')@foreach($tbs as $tabName => $table)@if(array_key_exists('relations', $table))
        {!! 'Schema::table(\''.$tabName.'\', function(Blueprint $table) {'!!}
